/* BIG BROTHER — Daily Cash Collection Mobile All-Locations Adapter V1 */
(function(){
  'use strict';
  ...
  async function parse(response){
    ...
    if(!response.ok)throw new Error(data.message||data.error_description||data.error||('Daily Cash mobile request failed ('+response.status+')'));
    return data;
  }
  ...
  async function ensureSession(){
    ...
    if(session.expires_at&&Number(session.expires_at)<Math.floor(Date.now()/1000)+30)await refreshSession();
    return session;
  }
  async function rpc(fn,args={}){
    await ensureSession();
    const call=()=>fetch(URL+'/rest/v1/rpc/'+fn,{
  ...
  async function api(params={}){
    ...
    switch(action){
      case 'invoiceList': return rpc('bb_dcc_mobile_invoice_list');
      case 'invoiceDetail': return rpc('bb_dcc_mobile_invoice_detail',{p_invoice_no:String(params.invoiceNo||'')});
      case 'invoiceHistoryRevision': {
        const revision=await rpc('bb_dcc_mobile_revision');
        return {success:true,revision:String(revision||'')};
      }
      case 'dccMyState': return rpc('bb_dcc_mobile_my_state');
      default: throw new Error('Unsupported Daily Cash mobile action: '+action);
    }
  }
  async function apiPost(params={}){
    const action=String(params.action||'');
    if(action==='dccCreateRequest')return rpc('bb_dcc_mobile_create_request',{p_payload:payload(params.requestData)});
    throw new Error('Unsupported Daily Cash mobile write action: '+action);
  }
  async function accessProfile(){return rpc('bb_dcc_mobile_access_profile')}
  async function locationMap(){
    const p=await accessProfile();const map={};
  ...
  window.BBDailyCashMobileAdapter={rpc,api,apiPost,ensureSession,accessProfile,locationMap};
})();
