/* BIG BROTHER — Sales Support Your Collection Supabase Adapter V1 */
(function(){
  'use strict';
  ...
  async function parse(response){
    ...
    if(!response.ok){
      throw new Error(data.message||data.error_description||data.error||('Your Collection database request failed ('+response.status+')'));
    }
    return data;
  }
  ...
  function payload(value){
    ...
    try{return JSON.parse(value)}catch(_){throw new Error('Invalid Your Collection request data.');}
  }
  ...
  async function api(params={}){
    ...
    switch(action){
      case 'invoiceList':
        return rpc('bb_sales_support_your_collection_invoice_list');
      case 'invoiceDetail':
        return rpc('bb_sales_support_your_collection_invoice_detail',{p_invoice_no:String(params.invoiceNo||'')});
      case 'invoiceHistoryRevision': {
        const revision=await rpc('bb_sales_support_your_collection_revision');
        return {success:true,revision:String(revision||'')};
      }
      case 'dccMyState':
        return rpc('bb_sales_support_your_collection_my_state');
      default:
        throw new Error('Unsupported Your Collection action: '+action);
    }
  }
  ...
  async function apiPost(params={}){
    ...
    if(action==='dccCreateRequest'){
      return rpc('bb_sales_support_your_collection_create_request',{p_payload:payload(params.requestData)});
    }
    throw new Error('Unsupported Your Collection write action: '+action);
  }

  async function accessProfile(){
    return rpc('bb_sales_support_your_collection_access_profile');
  }
  ...
  window.BBYourCollectionAdapter={rpc,api,apiPost,ensureSession,accessProfile,locationMap};
})();
